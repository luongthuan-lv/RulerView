...
class IapConnector @JvmOverloads constructor(
    ...
    init {
        ...
        mBillingService = BillingService(
            contextLocal, nonConsumableKeys, consumableKeys, subscriptionKeys
        )
    ...
    fun getListPurchasesDone(productServiceListener: ProductServiceListener) {
        getBillingService().getListPurchasesDoneListener(productServiceListener)
    }
    ...
    fun unPurchase(purchaseToken: String, listener: ConsumeResponseListener) {
        getBillingService().unPurchase(purchaseToken, listener)
    }
