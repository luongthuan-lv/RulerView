...
abstract class IBillingService {
    ...
    private val billingClientConnectedListeners: MutableList<BillingClientConnectionListener> =
        mutableListOf()
    private val productServiceListeners: MutableList<ProductServiceListener> = mutableListOf()
    ...
    fun getListPurchasesDoneListener(productServiceListener: ProductServiceListener) {
        productServiceListeners.add(productServiceListener)
    }
    ...
    fun productDoneOwned(listPurchaseInfo: List<DataWrappers.PurchaseInfo>?) {
        findUiHandler().post {
            if (listPurchaseInfo != null) {
                for (productServiceListener in productServiceListeners) {
                    productServiceListener.onGetListBillingDone(listPurchaseInfo)
                }
            } else {
                for (productServiceListener in productServiceListeners) {
                    productServiceListener.onGetListBillingDone(emptyList())
                }
            }
        }
    }
    ...
    abstract fun unPurchase(purchaseToken: String, listener: ConsumeResponseListener)
